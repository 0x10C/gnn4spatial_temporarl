...
class UCIDataGenerator():
    ...
    def __init__(self, npz_file_path):
        self.npz = np.load(npz_file_path)
        self.train_data = self.npz["train_feature"]
        self.train_targets = self.npz["train_targets"]
        self.val_data = self.npz["val_feature"]
        self.val_targets = self.npz["val_targets"]

        self.train_data = signal.resample(self.train_data, 3000, axis=1).transpose(0, 2, 1)
